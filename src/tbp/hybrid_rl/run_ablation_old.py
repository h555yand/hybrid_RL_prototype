...
def main() -> None:
    ...
    if RUN_EVAL:
        ...
        for mesh_name in EVAL_MESHES:
            ...
            # Сохранить результат для этого меша
        ...
        if all_bc_transitions:
            ...
            for tr in all_bc_transitions:
                # По мешам
                mesh_id = tr.mesh_id
                mesh_label = EVAL_MESHES[mesh_id] if 0 <= mesh_id < len(EVAL_MESHES) else f"unknown_{mesh_id}"
    ...
    if RUN_BC_TRAIN:
        ...
        # Статистика по мешам и типам
        type_names = ExperienceExtractor.get_type_names()
        mesh_names = {0: "cube", 1: "cylinder", 2: "mug", 3: "cup"}
        mesh_counts = {}
        type_counts = {}
        for tr in bc_transitions:
            mesh_label = mesh_names.get(tr.mesh_id, f"unknown_{tr.mesh_id}")
            mesh_counts[mesh_label] = mesh_counts.get(mesh_label, 0) + 1
            name = type_names.get(tr.action_type, f"type_{tr.action_type}")
            type_counts[name] = type_counts.get(name, 0) + 1
        print(f"Per mesh: {mesh_counts}")
        ...
        for i in range(min(10, len(bc_transitions))):
            ...
            mesh_label = mesh_names.get(tr.mesh_id, "?")
            print(
    ...
    if RUN_SAC_TRAIN:
        ...
        for mesh_name in SAC_MESHES:
            ...
            trainer.train(
                ...
                curriculum_levels=CURRICULUM_LEVELS,
                episode_pools=sac_train_pools[TRAIN_SEEDS[1]],
            )
    ...
    if RUN_SAC_EVAL:
        ...
        for mesh_name in SAC_EVAL_MESHES:
            ...
            results = _run_sac_eval(
                "sample", sac_trainer, sac_eval_pools, mesh_path, cfg
            )
    ...
    if RUN_ADAPTIVE:
        ...
        ADAPTIVE_MESH = "cup"  # параметр — какой меш адаптировать
        ...
        # Q-store: загружаем adaptive если есть, иначе unified
        ...
        if (adaptive_q_dir / "config.json").exists():
            ...
            q_load_dir = f"{str(runs_dir / 'unified_q')}_seed_{TRAIN_SEEDS[0]}"
        ...
        # SAC: загружаем adaptive если есть, иначе от тренировки
